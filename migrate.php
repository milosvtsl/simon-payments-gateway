...
foreach(array('court' => 'courtpay', 'utility_live' => 'utilitypay') as $old_schema => $schema) {
    ...
    $params = array();
    $sql = "SELECT * FROM {$old_schema}.transactions t WHERE EXISTS (SELECT * FROM {$old_schema}.merchants m WHERE t.pay_type = 1 AND m.id = t.merchant_id) ORDER BY ID DESC LIMIT {$limit}";
    $StatsQuery = $DB->prepare($sql);
    $StatsQuery->execute($params);
